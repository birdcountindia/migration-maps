gg_migrate <- function(
    spec1, pos_gr, pos_im, name_col, dot_size, 
  ...
  temp_title <- image_blank(2500, 100, "none") %>%
    image_annotate(text = spec1, font = "Gill Sans", size = 50, color = plot_col, 
      gravity = "center", weight = 900) %>%
    image_trim()
  ...
  for (k in seq_along(starts)) {
    ...
    p_map <- basemap + 
      geom_sf(data=filter(data_sf, DAY.Y %in% days),
              col=plot_col, size= dot_size, alpha=0.8, stroke=0) +
  ...
  final <- frames %>%
    ...
    image_composite(image_scale(title_card, "x35"), gravity="north", offset = "+0+20") %>%
    image_composite(image_scale(combined_logo, "x45"), offset = paste0("+", if(pos_gr == "R") 20 else 360, "+635")) %>%
    image_animate(fps=plot_fps)
  image_write(final, paste0("outputs/", spec1,"Migration Map ",print,".gif"))
